...
function App() {
  ...
  const totalSizeInMb = (files.reduce((acc, file) => acc + file.size, 0) / (1024 * 1024)).toFixed(2)
    ...
    <div className="page-shell">
      <main className="app-card">
        <header className="hero">
          <div>
            <p className="eyebrow">Smart Utility</p>
            <h1>🎬 Video to PDF Converter</h1>
            <p className="subtitle">Multiple videos ko ek hi PDF timeline me convert karo — fast, private, browser ke andar.</p>
          </div>
        ...
        <section className="stats-strip" aria-label="Quick stats">
          <div className="stat-tile">
            ...
            <strong>{files.length}/20</strong>
          </div>
          <div className="stat-tile">
            <span>Total Upload Size</span>
            <strong>{totalSizeInMb} MB</strong>
          </div>
          <div className="stat-tile">
        ...
        <form onSubmit={handleSubmit} className="form-layout">
          ...
          >
            ...
            <div className="upload-content">
              ...
              <p>Drag & drop ya button se files select karo.</p>
              <button
                type="button"
              ...
              <small>Max 20 videos • MP4 only</small>
            </div>
          </section>

          <section className="control-panel">
            <label htmlFor="fps-select">Frame extraction intensity</label>
            <div className="fps-headline">
              <span>FPS: </span>
              <strong>{fps}</strong>
            </div>
            <input
            ...
            <section className="files-list">
              ...
              <div className="files-grid">
                {files.map((file, index) => (
                  <article key={index} className="file-item">
                    <span className="file-name" title={file.name}>
                      {index + 1}. {file.name}
                    </span>
                    <span className="file-size-small">{(file.size / (1024 * 1024)).toFixed(1)} MB</span>
                    {!loading && (
                      <button
                        type="button"
