...
- **Obsidian files (Phase 1):** a leading YAML frontmatter block (`title`, `subject`, `tags`, …) is detected, kept as inert metadata (a frontmatter `title` becomes the document title), and never rendered as document content. Remaining Obsidian-only syntax — `[[Wikilinks]]`, `[[Page#Heading]]`, `![[embeds]]`, `#tags`, `^block-ids` — currently renders as the literal text you wrote; it is never silently dropped or corrupted. Wikilink resolution, embeds, and tag rendering are planned for later phases.
